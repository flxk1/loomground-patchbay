...
"""Real render test for the rebuilt Governance Patchbay (the new home app).

Boots serve.py, seeds a folder (bot7 wired to uc-draft auto; uc-decide reserved),
then runs patchbay_render.mjs which loads the actual index.html in jsdom against
the running shim and asserts the patchbay renders + a wire writes through.

  python3 app/patchbay_render_test.py
"""
...
tmp = tempfile.mkdtemp(prefix="patchbay_")
...
def setup():
    ...
    S.workspace_workflow("use_case_register", {"folder_context": F, "use_case_id": "uc-decide", "name": "uc-decide",
                                          ...
                                          "allowed_agents": [], "actor": "operator"})
    # post-G2: a use case reserves only from an AUTHORED reserve (legal enum dropped).
    # Author it, then run it so the reserved verdict projects onto the egress cord.
...
def main() -> int:
    ...
    time.sleep(0.3)
    try:
        r = subprocess.run(["node", str(HERE / "patchbay_render.mjs"), str(PORT), F],
